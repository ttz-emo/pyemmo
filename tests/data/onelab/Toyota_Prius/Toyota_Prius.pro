...
Group{
    Surf_cutA0 = Region[ {3, 1} ];
    Surf_cutA1 = Region[ {4, 2} ];
    Rotor_Bnd_MB = Region[ {6, 7, 8, 9, 10, 11, 12, 13} ];
    Rotor_Bnd_MB_1 = Region[ 6 ];
    Rotor_Bnd_MB_2 = Region[ 7 ];
    Rotor_Bnd_MB_3 = Region[ 8 ];
    Rotor_Bnd_MB_4 = Region[ 9 ];
    Rotor_Bnd_MB_5 = Region[ 10 ];
    Rotor_Bnd_MB_6 = Region[ 11 ];
    Rotor_Bnd_MB_7 = Region[ 12 ];
    Rotor_Bnd_MB_8 = Region[ 13 ];
    Rotor_Bnd_MBaux = Region[ {7, 8, 9, 10, 11, 12, 13} ];
    Rotor_Airgap = Region[ 31 ];
    Rotor_Magnets = Region[ {17, 19} ];
    RotorCC = Region[ {16, 18, 20, 31, 15, 28} ];
    Surf_Inf = Region[ 14 ];
    Stator_Bnd_MB = Region[ 5 ];
    Stator_Airgap = Region[ 30 ];
    StatorCC = Region[ {29, 30, 21, 22, 23, 24, 25, 26, 27} ];
    Stator_Ind_Ap = Region[ {22, 23} ];
    Stator_Ind_Cm = Region[ {24, 25} ];
    Stator_Ind_Bp = Region[ {26, 27} ];
    DomainNL = Region[ {15, 28, 21} ];
    DomainL = Region[ {16, 17, 18, 19, 20, 31, 22, 23, 24, 25, 26, 27, 29, 30} ];
    MovingBand_PhysicalNb = Region[ 0 ];
    Domain_Lam = Region[ {15, 21} ];
    DomainPlotMovingGeo = Region[ 32 ];
    group_PYEMMO_AIR = Region[ {6, 7, 8, 9, 10, 11, 12, 13, 31, 5, 30, 29, 0} ];
    group_MagnetPrius = Region[ {17, 19} ];
    group_Air = Region[ {16, 18, 20} ];
    group_M400_50A = Region[ {15, 28, 21} ];
    group_Copper2 = Region[ {22, 23, 24, 25, 26, 27} ];
    group_PYEMMO_AIR_dup = Region[ {6, 7, 8, 9, 10, 11, 12, 13, 31, 5, 30, 29, 0} ];
    group_MagnetPrius_dup = Region[ {17, 19} ];
    group_Air_dup = Region[ {16, 18, 20} ];
    group_M400_50A_dup = Region[ {15, 28, 21} ];
    group_Copper2_dup = Region[ {22, 23, 24, 25, 26, 27} ];
    }
...
Function{
    ...
    Mat_h_M400_50A = {0.0,100.0,150.0,180.0,200.0,250.0,300.0,350.0,450.0,
    550.0,650.0,750.0,850.0,950.0,1100.0,1250.0,1400.0,1550.0,
    1700.0,1900.0,2150.0,2450.0,2750.0,3150.0,3600.0,4100.0,4700.0,
    5250.0,6000.0,6700.0,7500.0,8650.0,9500.0,10750.0,14500.0,19500.0,
    25000.0,33000.0,44000.0,57000.0,74000.0,96000.0,130000.0,170000.0
    } ;
    Mat_b_M400_50A = {0.0,0.5,0.7,0.8,0.9,1.0,1.05,1.1,1.15,
    1.2,1.225,1.25,1.275,1.3,1.325,1.35,1.375,1.4,
    1.425,1.45,1.475,1.5,1.525,1.55,1.575,1.6,1.625,
    1.65,1.675,1.7,1.725,1.75,1.775,1.8,1.85,1.9,
    1.95,2.0,2.05,2.1,2.15,2.2,2.25,2.3
    } ;
...
Function{
    br[Region[17]] = 1*br_MagnetPrius * Vector[Cos[0.6974911925648072 + RotorPosition[]], Sin[0.6974911925648072 + RotorPosition[]], 0] ;
    br[Region[19]] = 1*br_MagnetPrius * Vector[Cos[0.0879069708326409 + RotorPosition[]], Sin[0.0879069708326409 + RotorPosition[]], 0] ;
    }
...
DefineConstant[
	...
	P_ = {"GetBOnRadius,GetMagnetLosses,Airgap_Force", Name "GetDP/2PostOperationChoices", Visible Flag_Debug || Flag_ExpertMode}
];
...
PostOperation{
    { Name GetBOnRadius; NameOfPostProcessing MagStaDyn_a_2D; 
         ...
         Operation {  
            Print [ b_radial, OnGrid {(0.08064999999999999*(1+0.0001))*Cos[$A*Pi/180],(0.08064999999999999*(1+0.0001))*Sin[$A*Pi/180],0}{0:360/SymmetryFactor:0.5,0,0}, File StrCat[ResDir,"b_radial_airgap_stator.pos"], Name "b_radial (airgap stator)" ]; 
            }
         Operation {  
            Print [ b_tangent, OnGrid {(0.08064999999999999*(1+0.0001))*Cos[$A*Pi/180],(0.08064999999999999*(1+0.0001))*Sin[$A*Pi/180],0}{0:360/SymmetryFactor:0.5,0,0}, File StrCat[ResDir,"b_tangent_airgap_stator.pos"], Name "b_tangent (airgap stator)" ]; 
            }
         Operation {  
    ...
    { Name Airgap_Force; NameOfPostProcessing MagStaDyn_a_2D; 
         Operation {  
            Print [ Force_MST, OnElementsOf Rotor_Bnd_MB_1, File StrCat[ResDir,"F_airgap.pos"], LastTimeStepOnly  ]; 
            Print [ Force_MST_Cyl, OnElementsOf Rotor_Bnd_MB_1, File StrCat[ResDir,"F_cyl_MB.pos"], Format SimpleTable, LastTimeStepOnly  ]; 
            Print [ 
                Force_MST_Cyl, 
                OnGrid {(0.08049999999999999*(1-0.0001))*Cos[$A*Pi/180],(0.08049999999999999*(1-0.0001))*Sin[$A*Pi/180],0}{0:360/SymmetryFactor:0.5,0,0}, 
                Name "Force (MST) airgap interp",
                File StrCat[ResDir,"F_cyl_airgap.pos"], 
                Format SimpleTable, 
                LastTimeStepOnly
            ]; 
            Print [ Force_MST_Rad, OnElementsOf Rotor_Bnd_MB_1, File StrCat[ResDir,"F_rad_airgap.pos"], LastTimeStepOnly  ]; 
            Print [ Force_MST_Tan, OnElementsOf Rotor_Bnd_MB_1, File StrCat[ResDir,"F_tan_airgap.pos"], LastTimeStepOnly  ]; 
            }
    }
    }
