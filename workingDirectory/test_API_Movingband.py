...
MaschineSurfList = createMachineGeometryFromSegment(dataFrameDict, extendInfo.symmetriefaktor[0])
...
for key in MaschineSurfaceDict.keys():
    for line in MaschineSurfaceDict[key].getCurve():
        if (abs(line.getP1().calcDist()-mbRadius) < pinball) and (abs(line.getP2().calcDist()-mbRadius) < pinball):
            lineCount += 1
            mbLines.append(line)
            nbrSegRotor = int(dataFrameDict[key].Quantity[0]/symFaktor) # we made sure its the rotor mb line so this must be the number of rotor-segments
...
for surface in MaschineSurfList:
    for line in surface.getCurve():
        ...
        if "LuA" in linename and "LuM" in linename:
            ...
            if "LuR" in surfaceName or "RoLu" in surfaceName: # its the rotor moving band line
