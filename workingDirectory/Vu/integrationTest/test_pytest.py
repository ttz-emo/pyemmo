...
class TestCases:
    ...
    def test_pyleecan(self):
        ...
        for test_id, test_case in self.test_cases[test_type].items():
            ...
            assert os.path.isfile(source_path) and messagePrinter(
                "SUCCESS: Source file exist"
        ...
        for test_id, test_case in self.test_cases[test_type].items():
            ...
            self.check_content(
                base_result_path, result_path[test_id], target_file_types
            )

            # 4.2 In simulation folder
            self.check_content(
                base_simul_path, simul_path[test_id], target_file_types
            )
    ...
    def make_test_cases(self, test_type: str) -> dict:
        ...
        test_files = glob.glob(
            os.path.join(ROOT_DIR, r"tests\data", test_type, "*.json")
        )
        print(test_files)
        test_names = list(
            map(
...
#     test_case_obj = TestCases()
#     test_case_obj.test_pyleecan()
